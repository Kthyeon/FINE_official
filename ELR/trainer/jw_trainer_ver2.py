...
#                 prgoress.set_description_str(f'Train epoch {epoch}')
...
#                         prob = torch.softmax(output, dim=-1)
                ...
                _, small_loss_idx = small_loss.topk(500, dim=-1)
                print (small_loss_idx)
                
#                 print (small_loss.topk(10, dim=-1)[0])
                print ('################')
                print (small_loss.topk(500, dim=-1)[0].shape)
                print (-small_loss.topk(500, dim=-1)[0][:, -1])
...
#                 small_loss_instances = torch.index_select(small_loss_instances, 1, small_loss_idx)
                ...
                print (small_loss_instances.shape)
                print (small_loss_instances[1])
                print (small_loss_instances[9])
                
                # TODO: mean을 내는 거를 softmax 값으로 할건지, 아니면 logit으로 할건지.
                r = torch.softmax(small_loss_instances.mean(dim=1), dim=-1)
#                 r = torch.softmax(small_loss_instances, dim=-1).mean(dim=1)
#                 r = small_loss_instances.mean(dim=1)
                print (r)
...
#             self.dataset = torch.utils.data.Subset(self.data_loader.dataset, neither_idxs + clean_idxs)
