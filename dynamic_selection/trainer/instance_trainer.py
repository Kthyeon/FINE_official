...
class InstanceTrainer(BaseTrainer):
    """
    ...
    """
    def __init__(self, model, model2, train_criterion, metrics, optimizer, config, data_loader, parse,
...
#             self.train_criterion = CCELoss()
        ...
        with tqdm(self.dynamic_train_data_loader) as progress:
            for batch_idx, (data, label, indexs, gt) in enumerate(progress):
                ...
                '''
                Train with instance dependent noise label
                '''
                with torch.no_grad():
                    _, label = self.model2(data)
